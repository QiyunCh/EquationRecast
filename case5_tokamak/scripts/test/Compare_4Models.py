...
evaluate relative L2 on delta T_e in the PHYSICAL domain over the SAME 20
randomly chosen val samples. Output: one grouped bar chart (linear + log y-scale).
...
Pipeline mirrors Plot_FNO.py:
  ...
  -> compare dT_pred with dT_true = Benchmark - IC
  -> map_back via HM -> physical grid -> relative L2 on delta T_e
"""
...
CONFIG = {
    ...
    # For delta Te, the denominator may be much smaller than full Te.
    # Keep this unchanged unless you want to include all nonzero delta Te values.
    "T_floor":        1e-4,
    ...
    "out_dir":        Path("plot/compare_4models_deltaTe"),
}
...
def build_val_indices_per_prefix(
    stats: Dict,
    data_h5: Path,
...
# ======================================================================
# INFERENCE  (single forward, returns disk-grid delta Te pred / true)
# ======================================================================
...
def run_inference(model, device, stats, raw, mask) -> Dict[str, np.ndarray]:
    ...
    # Predicted delta Te from the neural operator.
    dT_pred = denormalize_label(pred_norm[0], stats)

    # True physical delta Te.
    # This avoids the large-background Te issue in full Te comparison.
    dT_true = raw["Benchmark"] - raw["IC"]
    ...
    return {
        "dT_pred_disk": dT_pred,
        "dT_true_disk": dT_true,
    }


# ======================================================================
# ERROR METRIC  (relative L2 in physical domain, with floor)
# ======================================================================

def relative_L2_phys(
    pred_phys: np.ndarray,
    true_phys: np.ndarray,
    T_floor: float,
) -> float:
    valid = (
        np.isfinite(pred_phys)
        & np.isfinite(true_phys)
        & (np.abs(true_phys) >= T_floor)
    )

    if not valid.any():
        return float("nan")

    diff = (pred_phys - true_phys)[valid]
    den  = true_phys[valid]
...
def main():
    ...
    for mcfg in cfg["models"]:
        ...
        for disp, pfx in cfg["prefixes"]:
            ...
            for gidx in chosen[pfx]:
                ...
                dT_pred_phys = field_to_physical(
                    res["dT_pred_disk"],
                    phys_cache[pfx],
                )
                dT_true_phys = field_to_physical(
                    res["dT_true_disk"],
                    phys_cache[pfx],
                )
            ...
            print(
                f"  {mcfg['name']:>10s} | "
                f"{disp:>16s} | "
                f"mean rel L2 on delta Te = {mean_err:.4e}"
            )
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["model", "prefix", "gidx", "case_id", "rel_L2_deltaTe"])
    ...
    # --- save summary JSON ---
    summary = {
        "metric": "relative_L2_on_delta_Te_physical",
        "definition": "||deltaTe_pred - deltaTe_true||_2 / ||deltaTe_true||_2",
        "deltaTe_true": "Benchmark - IC",
        "param_counts": param_counts,
        "mean_rel_L2_deltaTe": {
            f"{m}__{p}": v for (m, p), v in results.items()
        },
        "n_samples_per_prefix": {
            disp: len(chosen[pfx])
            for disp, pfx in cfg["prefixes"]
        },
        "T_floor": cfg["T_floor"],
    ...
    for yscale in ("linear", "log"):
        ...
        ax.set_ylabel(
            r"Mean relative $L^2$ on $\Delta T_e$ "
            r"(physical, 20 val samples)"
        )
        ...
        out_png = out_dir / f"compare_4models_deltaTe_{yscale}.png"
