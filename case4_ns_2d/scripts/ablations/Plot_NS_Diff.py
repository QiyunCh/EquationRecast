#!/usr/bin/env python3
"""
Plot_NS_Diff.py — same layout as Plot_NS_Baseline.py (paper Fig. 4), but the
bottom field columns show the DIFFERENCE of each method from the benchmark
(3 columns: equation recast, parametric FNO, PINO).

Two figures are produced:
  - absolute difference  (pred - benchmark), vorticity units
  - relative difference  (pred - benchmark) / max|benchmark| * 100 (%)

Top error panels and the energy-spectrum column are identical to Fig. 4.
All recast quantities use the standard K_hard=21 recast. dpi=300.
"""
...
OUT_ABS = "results/Fig_Test3_NS_diff_abs.png"
OUT_REL = "results/Fig_Test3_NS_diff_rel.png"
...
# top error-panel curves
CURVES = [
    ("canonical_dataonly", "Equation recast", C_RECAST, "s"),
    ("parametric",         "Parametric FNO",  C_PARAM,  "o"),
    ("pino",               "PINO",            C_PINO,   "^"),
]
# spectrum curves (4, incl. benchmark)
SPEC_METHODS = [
    ("benchmark",          "Benchmark",       C_BENCH),
    ("canonical_dataonly", "Equation recast", C_RECAST),
    ("parametric",         "Parametric FNO",  C_PARAM),
    ("pino",               "PINO",            C_PINO),
]
# difference columns (3, method minus benchmark)
DIFF_METHODS = [
...
DETAIL_HSPACE = 0.32
DETAIL_WIDTH_RATIOS = [0.9, 0.9, 1.45, 1.45, 1.45]   # spectrum spans first two
DETAIL_WSPACE = 0.10
...
def radial_power_spectrum(a, rbin):
    a = a - a.mean()   # vorticity is zero-mean; drop spurious DC offset
    power = np.abs(np.fft.fft2(a)) ** 2
    rmax = int(rbin.max())
    return np.arange(rmax + 1), np.bincount(rbin.ravel(), weights=power.ravel(),
                                            minlength=rmax + 1)
...
def make_figure(mode, out_png, fh, Re, l2, rr, rbin, si, N):
    """mode: 'abs' or 'rel'."""
    def fetch(nm, Re_):
    ...
    # ---- top: two error panels (identical to Fig. 4) ----
    ...
    for row, Re_show in enumerate(RE_DETAIL):
        inner = gridspec.GridSpecFromSubplotSpec(
            1, 5, subplot_spec=outer[1 + row],
            width_ratios=DETAIL_WIDTH_RATIOS, wspace=DETAIL_WSPACE)

        # spectrum (spans first two columns) -- identical to Fig. 4
        ax_sp = fig.add_subplot(inner[0, 0:2])
        Ebench_max = 1.0
        for mid, lab, col in SPEC_METHODS:
        ...
        ax_sp.set_ylim(Ebench_max * 1e-7, Ebench_max * 3.0)
        ax_sp.set_xlim(0, 40)
        ...
        # difference fields (3 columns) -- absolute magnitude, min 0
        ref = fetch("benchmark", Re_show)
        bench_peak = float(np.max(np.abs(ref))) + 1e-30
        diffs = []
        for mid, _lab in DIFF_METHODS:
            d = fetch(mid, Re_show) - ref
            if mode == "rel":
                d = d / bench_peak * 100.0
            diffs.append(np.abs(d))                 # magnitude, no sign
        dmax = max(float(d.max()) for d in diffs)   # per-row shared scale
        ...
        cb.set_label("% of peak" if mode == "rel" else r"$|\Delta\omega|$", fontsize=16)
...
def main():
    ...
    make_figure("abs", OUT_ABS, fh, Re, l2, rr, rbin, si, N)
    make_figure("rel", OUT_REL, fh, Re, l2, rr, rbin, si, N)
    fh.close()
